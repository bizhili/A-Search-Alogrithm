...
int is_exist(list<block> table,int x,int y);
...
int main()
{
    ...
    int total_loop=0;
    block temp_block1;
    temp_block1.x=box_num-1;
    temp_block1.y=box_num-1;
    temp_block1.steps=0;
    ...
    while(table_o.size())
    {
        ...
        //drop traced node in open table
        table_o.erase(iter);
        ...
        check_and_do(x+1,y,table_o,table_t,steps,copy_data,empty_p);
        check_and_do(x,y+1,table_o,table_t,steps,copy_data,empty_p);
        check_and_do(x-1,y,table_o,table_t,steps,copy_data,empty_p);
        check_and_do(x,y-1,table_o,table_t,steps,copy_data,empty_p);
        check_and_do(x+1,y+1,table_o,table_t,steps,copy_data,empty_p);
        check_and_do(x+1,y-1,table_o,table_t,steps,copy_data,empty_p);
        check_and_do(x-1,y+1,table_o,table_t,steps,copy_data,empty_p);
        check_and_do(x-1,y-1,table_o,table_t,steps,copy_data,empty_p);
        ++total_loop;
    }
    //backtrace least cost road
...
void colorBox(Mat image,int x,int y,Scalar Color)
{
    rectangle(image,Point(x*19+1,y*19+1),Point(x*19+18,y*19+18),Color,CV_FILLED, 8, 0);
}
...
int check_and_do(int x,int y,list<block> &table_o,list<block> &table_t,int steps,uchar *copy_data,Mat image)
{
    if(y>=0 &&y<=box_num-1 && x<=box_num-1 && x>=0 &&copy_data[box_num*(x)+y]!=0 && is_exist(table_o,x,y)&&is_exist(table_t,x,y))
    {
        block temp_block;
        temp_block.x=x;
        temp_block.y=y;
        ...
        temp_block.score=sqrt(x*x+3*y*y)+steps+1;
        //temp_block.score=sqrt(x*x+3*y*y)/1000+steps+1;
        //temp_block.score=sqrt(x*x+3*y*y)*1000+steps+1;
        //temp_block.score=fabs(x)+fabs(y)+steps+1;
        //temp_block.score=fabs(x)+steps+1;
        //temp_block.score=fabs(y-x)+steps+1;
        //temp_block.score=fabs(y*x)+steps+1;
        //temp_block.score=fabs(x*x)+steps+1;
        //temp_block.score=fabs(x/(y+1))+steps+1;
